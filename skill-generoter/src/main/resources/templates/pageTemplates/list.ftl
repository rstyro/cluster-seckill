        ...
        <div class="box-header">
            ...
            <div class="search">
                <div class="input-group" style="float: right;width: 300px;">
                    <input type="text" class="form-control" id="keyword" th:value="<@notTran name="keyword" />"
                           placeholder="请输入关键字">
                    <span class="input-group-addon searcher" style="cursor:pointer;"><i
                                class="fa fa-search"></i> 搜素</span>
                </div>
            </div>
            <button th:onclick="add();" th:if="<@notTran name="QX.add == '1' && QX.query == '1'" />"
                    class="btn btn-success btn-sm"><i class="fa fa-plus"></i> &nbsp;&nbsp;新增
            </button>
        </div>
        <!-- /.box-header -->
        <div class="box-body">
            <div class="table-responsive">
                <table id="roleList" class="table table-bordered table-hover">
                    ...
                    <tbody>
                    <tr th:if="<@notTran name="QX.query == '1'" />"
                        th:each="item,stat :<@notTran name="list.data.records" />">
                        <td th:text="${r'${stat.count}'}">1</td>
                        <#list table.fields as column>
                            <#if column.propertyType == 'LocalDateTime'>
                                <td>
                                    <div th:text="<@notTran name="#temporals.format(item.${column.propertyName}, 'yyyy-MM-dd HH:mm:ss')"/>">${column.comment}</div>
                                </td>
                            <#else>
                        ...
                        <td>
                            <span th:unless="<@notTran name="QX.add == '1' || QX.del == '1' || QX.edit == '1' || QX.query == '1'"/>"><i
                                        class="fa fa-lock"></i> 无权限</span>
                            <span data-toggle="tooltip" data-placement="left" title="修改"
                                  th:if="<@notTran name="QX.edit == '1'"/>" th:data-id="<@gemTH name='id'/>"
                                  class="btn btn-xs btn-info" th:onclick="edit(this.getAttribute('data-id'));"><i
                                        class="fa fa-edit"></i></span>
                            <span data-toggle="tooltip" data-placement="right" title="删除"
                                  th:if="<@notTran name="QX.del == '1'"/>" class="btn btn-xs btn-danger"
                                  th:data-id="<@gemTH name='id'/>" th:onclick="del(this.getAttribute('data-id'));"><i
                    ...
                    <#list table.fields as column>
                        <#if column.propertyName != 'isDeleted' && !column.keyFlag>
                            <div class="form-group">
                                <label for="${column.propertyName}"
                                       class="col-sm-2 control-label"> <#if column.comment == ''>${column.propertyName}<#else>${column.comment}</#if></label>
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" name="${column.propertyName}" value=""
                                           id="${column.propertyName}"
                                           placeholder="请输入<#if column.comment == ''>${column.propertyName}<#else>${column.comment}</#if>">
                                </div>
                            </div>
                        </#if>
                    </#list>
        ...
        $("#submitBtn").click(function () {
            ...
            if (checkData(<#list table.fields as column><#if !column.keyFlag>${column.propertyName}<#if column_has_next>, </#if></#if></#list>)) {
                $.ajax({
                    ...
                    cache: false,
                    data: {
                <#list table.fields as column>${column.propertyName}:${column.propertyName},</#list>
